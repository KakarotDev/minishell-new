...
CFILES += $(addprefix $(AUXILIARY)/, free_struct_token.c ft_count_tokens.c ft_cpy_array_data.c ft_destructor_struct.c ft_free_ast.c \
	...
	ft_how_many_pipes.c ft_close_fds.c temp_functions.c) #eliminar posteriormente temp_functions.c
...
CFILES += $(addprefix $(EXECUTION)/, cmds_paths.c)
CFILES += $(addprefix $(EXPANSIONS)/, expansion_procedures.c send_token_to_expansion.c send_token_to_remove_quotes.c)
CFILES += $(addprefix $(LEXER)/, lexer.c generate_tokens.c quoting_validation.c)
